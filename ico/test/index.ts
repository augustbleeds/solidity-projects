...
describe("SpaceCoin", () => {
  ...
  describe("Token Contract", () => {
    ...
    it("Prevents non-owners from toggling on/off the 2% tax", async () => {
      expect(spaceCoin.connect(alice).setTax(true))
        .to.be.revertedWith("Non-owner not allowed to toggle tax");
    });
...
describe("SpaceCoin", () => {
  ...
  // const randomWallets = async (amount: number) => {

  //   console.log("hoarder before", await waffle.provider.getBalance(hoarder.address));

  //   // send test eth to hoarder
  //   for (let i = 0; i < investors.length; i++) {
  //     const tx = await investors[i].sendTransaction({to: hoarder.address, value: ethers.utils.parseEther("1050")});
  //     console.log(i, 'investor');
  //     await tx.wait();
  //   }

  //   console.log("hoarder after", await waffle.provider.getBalance(hoarder.address));

  //   const signers: Wallet[] = [];

  //   for (let i = 0; i < amount; i++) {
  //     const wallet = ethers.Wallet.createRandom();
  //     wallet.connect(waffle.provider);
  //     // send ETH to the new wallet so it can perform a txs
  //     await hoarder.sendTransaction({to: wallet.address, value: ethers.utils.parseEther("1010")});
  //     signers.push(wallet);
  //   }
  //   return signers
  // }
  ...
  describe("ICO Contract", () => {
    ...
    describe("Management", () => {
      it("Allows owner to advance phase forward", async () => {
        ...
        // general phase
        expect(await spaceCoinICO.advance());
