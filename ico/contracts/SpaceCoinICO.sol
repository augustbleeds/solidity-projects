...
contract SpaceCoinICO {
  ...
  uint256 constant public SEED_INDIVIDUAL_LIMIT = 1500 ether;
  uint256 constant public SEED_FUND_LIMIT = 15000 ether;
  uint256 constant public FUND_LIMIT = 30000 ether;
  uint256 constant public GENERAL_INDIVIDUAL_LIMIT = 1000 ether;
  ...
  function _redeem() internal {
    ...
    require(currentContributions[msg.sender] > 0, "SpaceCoinICO.sol: MUST_HAVE_CONTRIBUTIONS");
  ...
  function _checkValidityOfContribution() internal view {
    ...
    if(phase == Phase.SEED) {
      ...
      require(isSeedInvestor[msg.sender], "SpaceCoinICO.sol: SEED_PHASE_INVESTORS_ONLY");
    ...
    if((phase == Phase.GENERAL || phase == Phase.OPEN) && fundMax >= FUND_LIMIT) {
      revert FundingRoundFilled(msg.sender, phase);
    }
  ...
  function contribute() external contributeNotInProgress payable {
    ...
    if(msg.value + currentFund > fundLimit) {
      ...
      refundAdjustedByFundLimit = msg.value - actualContributionAdjustedByFundLimit;
    }
    ...
    if(phase == Phase.SEED || phase == Phase.GENERAL) {
      uint256 individualLimit = phase == Phase.SEED ? SEED_INDIVIDUAL_LIMIT : GENERAL_INDIVIDUAL_LIMIT;
      if(msg.value + currentContributions[msg.sender] > individualLimit) {
        actualContributionAdjustedByIndividualLimit = individualLimit - currentContributions[msg.sender];
        refundAdjustedByIndividualLimit = msg.value - actualContributionAdjustedByIndividualLimit;
      }

      // if the individual limit is triggered first, both are triggered at the same time, or neither is triggered
      if(actualContributionAdjustedByIndividualLimit <= actualContributionAdjustedByFundLimit) {
        currentContributions[msg.sender] += actualContributionAdjustedByIndividualLimit;
        purchased[msg.sender] += actualContributionAdjustedByIndividualLimit;
    ...
    currentContributions[msg.sender] += actualContributionAdjustedByFundLimit;
    ...
    if(refundAdjustedByFundLimit > 0) {
      (bool success, ) = msg.sender.call{value: refundAdjustedByFundLimit}("");
      require(success, "SpaceCoinICO.sol: REFUND_CALL_FAILED");
    }
  ...
  function addSeedInvestors(address[] memory investors) public onlyOwner contributeNotInProgress {
  ...
  function removeSeedInvestors(address[] memory investors ) public onlyOwner contributeNotInProgress {
    for (uint i = 0; i < investors.length; i++) {
      if(isSeedInvestor[investors[i]]) {
  ...
  function setPausedState(bool _newState) public onlyOwner contributeNotInProgress {
  ...
  // returns new phase
  function advance() public onlyOwner contributeNotInProgress returns (Phase) {
    if(phase == Phase.SEED) {
      phase = Phase.GENERAL;
    } else if (phase == Phase.GENERAL) {
      phase = Phase.OPEN;
    } else {
      // owner cannot manually advance state anymore
      revert("SpaceCoinICO.sol: CANT_ADVANCE_PAST_OPEN_PHASE");
    }
    emit PhaseAdvance(phase);
    return phase;
  }

  function setTax(bool tax) external onlyOwner contributeNotInProgress returns(bool) {
